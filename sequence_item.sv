...
// con4: VALID state transitions
    constraint current_state_transition {
      if (~comm_flag) { // only randomize when idle
        if (current_state == WRITE_ADD)
          MOSI_bits[10:8] dist {WRITE_DATA := 90, WRITE_ADD := 10};
        else if (current_state == WRITE_DATA)
          MOSI_bits[10:8] dist {READ_ADD := 98, WRITE_ADD := 2};
        else if (current_state == READ_ADD)
          MOSI_bits[10:8] dist {READ_DATA := 98, READ_ADD := 2};
        else if (current_state == READ_DATA)
          MOSI_bits[10:8] dist {WRITE_ADD := 50, READ_ADD := 50};
      }
    }
endclass 
endpackage